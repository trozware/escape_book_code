...
class AppState {
  ...
  private func fetchISSLocation() async -> ISSLocation? {
    do {
      let newLocation = try await networker.fetchISSDetails()
      return newLocation
    } catch FetchError.noResponse {
      logger.fault("No response from API server")
    } catch FetchError.badResponseCode(let code) {
      logger.fault("Bad response code from API server: \(code)")
    } catch FetchError.badJSON(let error) {
      logger.fault("Error decoding JSON from API server: \(error)")
    } catch {
      logger.fault("Unknown error (probably offline): \(error)")
    }
    return nil
  }
